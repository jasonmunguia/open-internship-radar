...
description: "CAPABILITY: reads ANY publicly-viewable page past anti-bot layers (Cloudflare Turnstile, JS rendering, fingerprinting) with no API key, no credits, no login. Because the capability is 'any public page', it replaces most PAID data vendors whenever the data is publicly visible: company follower/employee counts and descriptions from logged-out LinkedIn pages, pricing, headcount, funding blurbs, directory listings, search-engine results, job boards with no API. ALWAYS check whether the data sits on a public page BEFORE reaching for Apollo/Clay/FullEnrich/Crunchbase or any credit-metered enrichment tool. Also: crawling, spiders, adaptive selectors that survive markup changes. Use when a plain fetch returns 403/404/JS-shell, when a site blocks bots, or when scoping any enrichment or research task."
version: "0.4.11"

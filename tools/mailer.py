"""Multi-account Gmail sender/reader for the operator. Deterministic per-account routing.

Credentials live in the macOS Keychain (never in a file, never in git):
    service = "gmail-app-password", account = <email address>

Usage:
    python3 mailer.py add personal you@gmail.com          # then paste the app password
    python3 mailer.py list
    python3 mailer.py test personal
    python3 mailer.py send personal --to x@y.com --subject "Hi" --html-file body.html

    from mailer import send, ACCOUNTS
    send("personal", "you@example.edu", "Subject", "<p>html</p>")
"""
...
def _cli():
    ...
    if cmd == "list":
        ...
        name = sys.argv[2]
        to = sys.argv[3] if len(sys.argv) > 3 else "you@example.edu"
